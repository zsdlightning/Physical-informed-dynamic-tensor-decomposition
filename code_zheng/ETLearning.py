...
class SparseGP(Module):
    ...
    def forward_mean(self, X):
        ...
        # Ltril = torch.tril(self.L_g) # R x Z x Z
        # S = (Ltril @ Ltril.T).view((self.dim_output, 1, self.num_pseudo, self.num_pseudo)) # R x 1 x z x z
        # sigma = kXX - alphaT @ (kZZ - S) @ alpha # R x n x 1 x 1
        f = m #+ torch.empty_like(m).normal_() * torch.sqrt(sigma)
        f = f.view((-1, self.dim_output))
        return f
...
class ODEFunction(Module):
    ...
    def forward(self, t, u):
        ...
        s_idx = 0
        for k in range(self.nmod):
            e_idx = s_idx + self.nvec[k]
            react = self.reaction[k](X[s_idx: e_idx, :])
            # print(react.shape)
    ...
    #     KL = 0
    #     for k in range(self.nmod):
    #         KL += self.reaction[k].KL_divergence()
    #     return KL
...
class ETL(Module):
    ...
    def model_parameters(self, weights: typing.List[np.ndarray]) -> None:
        for parameter, sample in zip(self.parameters(), weights):
    ...
    def train(self, ind, t, y, ind_te, t_te, y_te, batch_size=100, test_every=100, total_epoch=100, lr=1e-3):
        ...
        scheduler = ReduceLROnPlateau(optimizer, 'min', min_lr=1e-3) 
        ...
        for epoch in tqdm(range(total_epoch)):
            ...
            if (epoch + 1) % test_every == 0:
                with torch.no_grad():
                    self.is_train = False
                    # self.ode_func.is_train = False
                    # current_parameters = self.model_parameters
                    test_batch_size = N_te
                    num_batch = (N_te + test_batch_size - 1) // test_batch_size
                    y_list = []
                    for i in range(num_batch):
                        batch_ind = ind_te[i * test_batch_size: (i+1)*test_batch_size]
                        batch_t = t_te[i * test_batch_size: (i+1)*test_batch_size]
                        batch_y = y_te[i * test_batch_size: (i+1)*test_batch_size]
                        pred_y = self.predict_(batch_ind, batch_t, 'dopri5')
                        y_list.append(pred_y)
                        # print(pred_y)
                        # se += ((batch_y - pred_y)**2).sum()
                        # ae += torch.abs(batch_y - pred_y).sum()
                    if len(y_list) > 1:
                        pred_y = torch.cat(y_list)
                    else:
                        pred_y = y_list[0]

                    # pred_y = pred_y.mean(dim=1).view((-1, 1))
                    nrmse = torch.sqrt(((pred_y - y_te)**2).mean()) / torch.sqrt((y_te**2).mean())
                    nmae = torch.abs(pred_y - y_te).mean() / torch.abs(y_te).mean()
                    ...
                    if len(y_list) > 1:
                        ...
                        pred_y = y_list[0]
                    nrmse = torch.sqrt(((pred_y - y)**2).mean()) / torch.sqrt((y**2).mean())
                    nmae = torch.abs(pred_y - y).mean() / torch.abs(y).mean()
                    print('Training: NRMSE: {} NMAE: {}'.format(nrmse, nmae))
                    scheduler.step(nrmse)
                    # self.model_parameters = current_parameters

                    self.is_train = True
                    # self.ode_func.is_train = True

        return nrmse_list, nmae_list
    ...
    def predict_(self, batch_ind, batch_t, method='euler'):
        batch_size = batch_ind.shape[0]
        ...
        for k in range(self.nmod):
            idx = np.sum(self.nvec[:k]).astype(np.int64) + batch_ind[:, k]
            idx = idx.long()
            # embedding_prod *= e[np.arange(batch_size).astype(np.int64), idx].view((batch_size, self.dim_embedding))
            embeddings.append(e[np.arange(batch_size).astype(np.int64), idx].view((batch_size, self.dim_embedding)))
