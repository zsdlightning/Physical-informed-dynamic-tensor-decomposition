...
class Bayes_diffu_tensor:
    ...
    def msg_update_U_llk(self, T):
        ...
        uid_table, _ = build_id_key_table(
            self.nmod, ind_T
        )  # get the id of associated nodes
        uid_list = []
        ...
        U_llk_del_T = torch.cat(
            [self.msg_U_llk_m_del[:, :, T], self.msg_U_llk_v_del[:, :, T]]
        )  # concat the m and v
        U_llk_del_T.requires_grad = True

        U_llk_del_T_m, U_llk_del_T_v = self.arrange_U_llk(
            U_llk_del_T
        )  # arrange U as mode-wise

        E_z_del, E_z_2_del = self.moment_product_U_del(
            ind_T, U_llk_del_T_m, U_llk_del_T_v
        ...
        # ADF update
        U_llk_m_star = (
            self.msg_U_llk_m_del[:, :, T]
            + self.msg_U_llk_v_del[:, :, T] * U_llk_del_m_grad
        )
        U_llk_v_star = self.msg_U_llk_v_del[:, :, T] - torch.square(
            self.msg_U_llk_v_del[:, :, T]
        ...
        msg_U_llk_v_inv_new = (
            1.0 / U_llk_v_star[uid] - 1.0 / self.msg_U_llk_v_del[uid, :, T]
        )
        msg_U_llk_v_inv_m_new = torch.div(
            U_llk_m_star[uid], U_llk_v_star[uid]
        ) - torch.div(self.msg_U_llk_m_del[uid, :, T], self.msg_U_llk_v_del[uid, :, T])

        # Damping & neg-var check
        msg_U_llk_v_inv = (
            self.DAMPING * (1.0 / self.msg_U_llk_v[uid, :, T])
            + (1 - self.DAMPING) * msg_U_llk_v_inv_new
        )
        msg_U_llk_v_inv_m = (
            self.DAMPING
            * torch.div(self.msg_U_llk_m[uid, :, T], self.msg_U_llk_v[uid, :, T])
        ...
        self.msg_U_llk_m[uid, :, T] = torch.nan_to_num(
