...
def data_loader_dynamic_CP(args, data_file_name):
    ...
    if args.fix_int:
        ...
        time_int_list_follow = [
            train_time[i + 1] - train_time[i] for i in range(N_time - 1)
        ]
    ...
    F_base = torch.tensor([[0, 1.0], [-lamb * lamb, -2 * lamb]])
    P_inf_base = torch.tensor([[variance, 0.0], [0, lamb * lamb * variance]])
