...
if data_type == 'static':
    ...
    data_dict['tr_ind'] = full_data['train_ind']
    data_dict['te_ind'] = full_data['test_ind']

    
else:
    data_dict['ndims'] = full_data['ndims_time'] 
    data_dict['tr_ind'] = np.concatenate([full_data['train_ind'],full_data['train_time_disct'].reshape(-1,1)],1)
    data_dict['te_ind'] = np.concatenate([full_data['test_ind'],full_data['test_time_disct'].reshape(-1,1)],1)
    
data_dict['tr_y'] = torch.tensor(full_data['train_y']).double()
data_dict['te_y'] =  torch.tensor(full_data['test_y']).double()
data_dict['num_node'] = sum(data_dict['ndims'])
