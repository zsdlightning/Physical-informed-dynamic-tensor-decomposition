...
class CPD:
    ...
    def __init__(self, nvec, R, ns, max_epoch):
        ...
        self.U = torch.tensor(np.random.rand(self.ns, self.d, self.R), dtype=torch.float32, requires_grad=True) 
    ...
    def get_loss(self, train_ind, train_y):
        ll = -0.5*torch.sum(torch.square(self.U[0])) - 0.5*self.d*self.R*(self.ns - 1)*self.log_v
        ll = ll - 0.5*torch.exp(-self.log_v)*torch.sum(torch.square(self.U[1:] - self.U[0:-1]))
        ...
        ll = ll - 0.5*torch.exp(-self.log_tau)*torch.sum(torch.square(train_y- pred))
        return -ll
