...
class CPADF:
    def __init__(self, nvec, R):
        ...
        self.v = [
            torch.tensor(
                np.ones([nvec[j], self.R]), dtype=torch.float32, requires_grad=False
            )
            for j in range(self.nmod)
        ]
    ...
    def go_through(self, ind, y, test_ind, test_y):
        ...
        for npass in range(10):
            for n in range(N):
                ...
                # nmod * R X 1
                mu_n = (
                    torch.hstack([self.mu[j][ind_n[j]] for j in range(self.nmod)])
                    .clone()
                    .detach()
                    .requires_grad_(True)
                )
                v_n = (
                    torch.hstack([self.v[j][ind_n[j]] for j in range(self.nmod)])
                    .clone()
                    .detach()
                    .requires_grad_(True)
                )
